...
const HEADERS  = { 'User-Agent': 'cybersage-portfolio', Accept: 'application/vnd.github+json' };
...
export async function GET() {
  try {
    const [profileRes, eventsRes] = await Promise.all([
      fetch(`https://api.github.com/users/${USERNAME}`, { headers: HEADERS, next: { revalidate: 3600 } }),
      fetch(`https://api.github.com/users/${USERNAME}/events/public?per_page=15`, { headers: HEADERS, next: { revalidate: 3600 } }),
    ]);
